import { reqSongUrl, reqSongImg, reqSongLyric } from "../../utils/requset"
...
const changeSongDetailAction = (songdetail) => {
    return { type: "changeSongDetail", songdetail }
}
// 修改音乐歌词
const changeSongLyircAction = (songlyric) => {
    return { type: "changeSongLyric", songlyric }
}
...
export const reqSongAction = (id) => {
    return (dispatch, getState) => {
        ...
        reqSongImg({ ids: id }).then(res => {
            dispatch(changeSongDetailAction(res.data.songs[0]))
        })
        reqSongLyric({ id: id }).then(res => {
